...
export async function handleRequest(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const extractRequest = url.pathname === '/receipt/extract';

    if (!allowed) throw new SafeError('ORIGIN_DENIED', 403);
    if (!extractRequest && url.pathname !== '/health' && !sessionRequest) throw new SafeError('NOT_FOUND', 404);

    if (request.method === 'OPTIONS') {
      if ((!extractRequest && !sessionRequest) || request.headers.get('access-control-request-method') !== 'POST' ||
        (request.headers.get('access-control-request-headers') ?? '').split(',').some(h => h.trim() && !['accept', 'content-type', 'authorization'].includes(h.trim().toLowerCase()))) {
        throw new SafeError('CORS_DENIED', 403);
      }
      code = 'OK';
    ...
    if (request.method === 'GET' && url.pathname === '/health') {
      code = 'OK';
      return reply({ ok: true, service: 'receiptmind-api' });
    }
    if (request.method !== 'POST' || (!extractRequest && !sessionRequest)) throw new SafeError('METHOD_NOT_ALLOWED', 405);

    const ip = request.headers.get('cf-connecting-ip');
    if (!ip) throw new SafeError('CLIENT_ADDRESS_UNAVAILABLE', 503);
    const ipKey = await identity(env, 'ip', ip);
    // Count unauthenticated attempts too, so token guessing cannot bypass abuse controls.
    await consume(env, ipKey, 'requests', limit(env.IP_REQUESTS_PER_MINUTE), 'minute');

    // Every session and extraction request must prove a Google/Apple identity.
    ...
    if (sessionRequest) {
      code = 'OK';
      return reply({ expiresAt: signIn.expiresAt });
    }

    const contentType = request.headers.get('content-type') ?? '';
    if (!/^multipart\/form-data;\s*boundary=/i.test(contentType) || request.headers.has('content-encoding')) {
    ...
    if (!(entry instanceof File) || !entry.size || form.getAll('receipt').length !== 1 ||
      ...
      (form.has('privacy_mode') && form.get('privacy_mode') !== 'no-payment-data')) {
      throw new SafeError('INVALID_UPLOAD', 400);
    }
    if (entry.size > 10 * 1024 * 1024) throw new SafeError('UPLOAD_TOO_LARGE', 413);
    const mime = await validateFile(entry);

    // Cost protection is layered: authenticated-user monthly allowance plus
    // network-level daily abuse protection. Both are consumed before Gemini.
